...
class LbsApp:
    ...
    def download_tomlpp_lib(self, dst: str, url: str = 'https://github.com/marzer/tomlplusplus/raw/refs/heads/master/toml.hpp') -> None:
        if os.path.isfile(dst):
            self.info('toml++ found')
            return True
        else:
            self.info('Fetching toml++...')
            data = self.request_bin(url)
            open(dst, 'wb').write(data)
            return True
    ...
    def join_code(self, src_paths: any, inc_paths: any) -> str:
        # it doesn't handle same file names with different paths yet
        ret = '// This source code file is generated by LBS\n'
        ret = '// GoJoinUrCode' + b'\xe2\x84\xa2'.decode() + ' technology created by Pixelsuft\n'
        ret += '#define LBS_GO_JOIN_UR_CODE\n'
        inc_paths = [x.replace('\\', '/') for x in inc_paths]
        used_inc = []
        for fn in src_paths:
            data = open(fn, encoding='utf-8').read()
            for i in data.split('\n'):
                if i.lower().strip().startswith('#include'):
                    ret += self.add_inc_file(i, inc_paths, used_inc)
                    continue
                ret += i + '\n'
        return ret

    def add_inc_file(self, line: str, inc_paths: any, used_inc: list) -> str:
        ret = ''
        f = self.get_inc_path(self.gen_inc_fn(line), inc_paths)
        if f:
            if f in used_inc:
                return ret
            used_inc.append(f)
            for m in open(f, encoding='utf-8').read().split('\n'):
                if m.lower().strip().startswith('#pragma') and 'once' in m: continue
                if m.lower().strip().startswith('#include'):
                    ret += self.add_inc_file(m, inc_paths, used_inc)
                    continue
                ret += m + '\n'
            return ret
        ret += line + '\n'
    ...
    @staticmethod
    def get_inc_path(inc_fn: str, inc_paths: any) -> str:
        if not inc_fn:
            return ''
        for i in inc_paths:
            if i.split('/')[-1].lower() == inc_fn.split('/')[-1].lower():
                return i
        return ''

    @staticmethod
    def gen_inc_fn(inc_str: str) -> str:
        try:
            return inc_str.replace('<', '"').replace('>', '"').split('"')[1].replace('\\', '/')
        except IndexError:
            return ''
